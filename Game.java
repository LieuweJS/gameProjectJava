...
/**
 * this is the main class of the game, the Game class, this is the basis of everything.
 * @author Lieuwe Baron.
 * @version 23/01/2020.
 */
public class Game extends JFrame {
                    ...
                    if(map.chamberList.get(map.getCurrentFloor()).getGroundType(map.getPlayerY()/50,map.getPlayerX()/50).equals("Enemy") || map.chamberList.get(map.getCurrentFloor()).getGroundType(map.getPlayerY()/50,map.getPlayerX()/50).equals("Boss")) {
                        if(map.chamberList.get(map.getCurrentFloor()).enemies.get(map.chamberList.get(map.getCurrentFloor()).getEnemyId(map.getPlayerY()/50,map.getPlayerX()/50)).Health != 0) {
                            ...
                            if(input.equals("use attack")) {  
                                ...
                                int enemyChance1 = map.chamberList.get(map.getCurrentFloor()).enemies.get(map.chamberList.get(map.getCurrentFloor()).getEnemyId(map.getPlayerY()/50,map.getPlayerX()/50)).Attacks.get(0).chance;
                                int enemyChance2 = map.chamberList.get(map.getCurrentFloor()).enemies.get(map.chamberList.get(map.getCurrentFloor()).getEnemyId(map.getPlayerY()/50,map.getPlayerX()/50)).Attacks.get(1).chance;
                                ...
                                Combat combat = new Combat(map.player.Health,  map.player.damage, enemyHealth, enemyDamage1, enemyDamage2, enemyDamage3, enemyChance1, enemyChance2);
    ...
    class Canv extends JPanel {
        ...
        Map map;
        /**
         * this is the constructor for the Canv class
         * @param map: this input determines the Map instance used in the Canv instance.
         */
        public Canv(Map map) {
            this.map = map;
